...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      console.log(`Returning updated list with ${words.length} words.`);
  ...
  if (req.method === 'GET') {
    try {
      console.log("Attempting to lrange 'words' key...");
      const wordsList = await kv.lrange('words', 0, -1);
      const words = wordsList.map(word => JSON.parse(word));
      console.log(`Returning list with ${words.length} words.`);
